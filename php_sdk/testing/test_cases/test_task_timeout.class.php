...
class TestTaskTimeout extends TestAbstract
{
    ...
    public function test() 
    {
        ...
        if ($globals['MAX_LOCK_TIME'] == 0)
        {
            print "Skipping TestTaskTimeout is pointless because MAX_LOCK_TIME is set to infinite" . PHP_EOL;
        }
        else
        {
            $scheduler = $this->getScheduler();
            
            $not_important_id = $scheduler->addTask(
                $name="not_important", 
                $context=array(), 
                $dependencies=array(),
                $priority=1
            ...
            $high_importance_id = $scheduler->addTask(
